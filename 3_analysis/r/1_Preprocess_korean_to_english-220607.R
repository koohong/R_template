...
saveRDS(roadWidth,paste0(rawPath,"road_width_2016.rds"))
...
saveRDS(emergencyMedical,paste0(rawPath,"emergency_medical_2016-rvsd.rds"))
...
## Column 10. drunken
pedestrianAccident_drunken <- data.frame(drunkenEng = c("no","yes"), drunkenKor = (unique(pedestrianAccident$drunken)))

for(i in 1:nrow(pedestrianAccident_drunken)){
  pedestrianAccident$drunken[(pedestrianAccident$drunken %in% pedestrianAccident_drunken$drunkenKor[i])] <- pedestrianAccident_drunken$drunkenEng[i]
}

# unique(pedestrianAccident$drunken)

## Column 11. drunken
# pedestrianAccident_alcohol <- data.frame(alcoholEng = c("","0.15%~0.19%","0.05%~0.09%"), alcoholKor = (unique(pedestrianAccident$alcochol_degree)))

pedestrianAccident$alcochol_degree[(pedestrianAccident$alcochol_degree %in% "측정불응")] <- "reject_measurement"
pedestrianAccident$alcochol_degree[(pedestrianAccident$alcochol_degree %in% "0.35%이상")] <- "over_0.35%"

# unique(pedestrianAccident$alcochol_degree)
...
## Column 13. victim_sex

for(i in 1:nrow(pedestrianAccident_perpSex)){
  pedestrianAccident$victim_sex[(pedestrianAccident$victim_sex %in% pedestrianAccident_perpSex$perpSexKor[i])] <- pedestrianAccident_perpSex$perpSexEng[i]
}


## Column 14. victim_age

pedestrianAccident$victim_age[which(substr(pedestrianAccident$victim_age,start = 2,stop = 2) == "세")] <- substr(pedestrianAccident$victim_age,start = 1,stop = 1)[which(substr(pedestrianAccident$victim_age,start = 2,stop = 2) == "세")]
#length(substr(pedestrianAccident$victim_age,start = 1,stop = 1)[which(substr(pedestrianAccident$victim_age,start = 2,stop = 2) == "세")])
#length(pedestrianAccident$victim_age[which(substr(pedestrianAccident$victim_age,start = 2,stop = 2) == "세")])
#unique(pedestrianAccident$victim_age)

pedestrianAccident$victim_age[which(pedestrianAccident$victim_age %in% "불명")] <- "unidentified"

## Column 15. severity
pedestrianAccident_severity <- data.frame(severityEng = c("major_injury","minor_injury","inury_report","death","no_injury","unidentified"), severityKor = (unique(pedestrianAccident$severity)))

#unique(pedestrianAccident$severity)

for(i in 1:nrow(pedestrianAccident_severity)){
  pedestrianAccident$severity[(pedestrianAccident$severity %in% pedestrianAccident_severity$severityKor[i])] <- pedestrianAccident_severity$severityEng[i]
}

## Column 16. road type
pedestrianAccident_roadType <- data.frame(roadTypeEng = c("metropolitan_city_road","etc","Gun_road","Si_road","local_highway","general_national_highway","national_expressway"), roadTypeKor = (unique(pedestrianAccident$road_type)))

for(i in 1:nrow(pedestrianAccident_roadType)){
  pedestrianAccident$road_type[(pedestrianAccident$road_type %in% pedestrianAccident_roadType$roadTypeKor[i])] <- pedestrianAccident_roadType$roadTypeEng[i]
}

## Column 17. road shape
pedestrianAccident_roadShape <- data.frame(roadShapeEng = c("mid-block","intersection","etc/unidentified"), roadShapeKor = (unique(pedestrianAccident$road_shape)))

# unique(pedestrianAccident$road_shape)

for(i in 1:nrow(pedestrianAccident_roadShape)){
  pedestrianAccident$road_shape[(pedestrianAccident$road_shape %in% pedestrianAccident_roadShape$roadShapeKor[i])] <- pedestrianAccident_roadShape$roadShapeEng[i]
}

## Column 18. road alignment
pedestrianAccident_roadAlign <- data.frame(roadAlignEng = c("flat","downhill","uphill","etc"), roadAlignKor = (unique(pedestrianAccident$road_alignment)))

# unique(pedestrianAccident$road_shape)

for(i in 1:nrow(pedestrianAccident_roadAlign)){
  pedestrianAccident$road_alignment[(pedestrianAccident$road_alignment %in% pedestrianAccident_roadAlign$roadAlignKor[i])] <- pedestrianAccident_roadAlign$roadAlignEng[i]
}

## Column 19. junction shape
pedestrianAccident_juncShape <- data.frame(juncShapeEng = c("not_a_intersection","3-direction","4-direction","round","5-direction","etc/unidentified"), juncShapeKor = (unique(pedestrianAccident$junction_shape)))

# unique(pedestrianAccident$road_shape)

for(i in 1:nrow(pedestrianAccident_juncShape)){
  pedestrianAccident$junction_shape[(pedestrianAccident$junction_shape %in% pedestrianAccident_juncShape$juncShapeKor[i])] <- pedestrianAccident_juncShape$juncShapeEng[i]
}

## Column 20. road surface
pedestrianAccident_roadSurf <- data.frame(roadSurfEng = c("paved","unpaved"), roadSurfKor = (unique(pedestrianAccident$road_surface)))

#unique(pedestrianAccident$road_surface)

for(i in 1:nrow(pedestrianAccident_roadSurf)){
  pedestrianAccident$road_surface[(pedestrianAccident$road_surface %in% pedestrianAccident_roadSurf$roadSurfKor[i])] <- pedestrianAccident_roadSurf$roadSurfEng[i]
}

## Column 21. road surface condition
pedestrianAccident_roadSurfCon <- data.frame(roadSurfConEng = c("dry","wet","etc","snow","frost","thaw","flooding"), roadSurfConKor = (unique(pedestrianAccident$road_surface_condition)))

#unique(pedestrianAccident$road_surface)

for(i in 1:nrow(pedestrianAccident_roadSurfCon)){
  pedestrianAccident$road_surface_condition[(pedestrianAccident$road_surface_condition %in% pedestrianAccident_roadSurfCon$roadSurfConKor[i])] <- pedestrianAccident_roadSurfCon$roadSurfConEng[i]
}

## Column 22. weather
pedestrianAccident_weather <- data.frame(weatherEng = c("clean","cloudy","etc/unidentified","fog","rainny","snowy"), weatherKor = (unique(pedestrianAccident$weather)))

#unique(pedestrianAccident$weather)

for(i in 1:nrow(pedestrianAccident_weather)){
  pedestrianAccident$weather[(pedestrianAccident$weather %in% pedestrianAccident_weather$weatherKor[i])] <- pedestrianAccident_weather$weatherEng[i]
}


saveRDS(pedestrianAccident,paste0(rawPath,"accident_2016_2020.rds"))
